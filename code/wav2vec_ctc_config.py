...
class Wav2VecConvExtractorMode(Enum):
    """
    Mode for feature extractor. group_norm has a single group norm with d groups in the first conv block,
...
class Wav2VecDecoderConfig:
    _target_: str = 'wav2vec_ctc_models.Wav2VecLinearDecoder'
    feat_in: int = MISSING
...
class ConvFeatureEncoderConfig:
    extractor_mode: Wav2VecConvExtractorMode = field(default=Wav2VecConvExtractorMode.layer)
...
class Wav2VecMaskingConfig:
    ...
    mask_min_space: int = field(
        default=2, metadata={'help': 'Minimum space beetween spans (if no overlap is enabled)'}
    )
    ...
    mask_channel_min_space: int = field(
        default=2, metadata={'help': 'Minimum space between spans (if no overlap is enabled)'}
    )


@dataclass
class Wav2VecCTCConfig:
    ...
    decoder: Wav2VecDecoderConfig = Wav2VecDecoderConfig()
    ...
    feature_grad_mult: float = field(default=0.0, metadata={'help': 'Multiply extracted feature gradients'})
