...
#SBATCH -J "swdl:::langspeech:Wav2Vec-CTC-LibriSpeech_group"     # job name (<< CHANGE ! >>)
#SBATCH --exclusive             # exclusive node access
#SBATCH --mem=0                 # all mem avail
#SBATCH --mail-type=FAIL        # only send email on failure
#SBATCH --ntasks-per-node=8     # n tasks per machine (one task per gpu)
#SBATCH --cores-per-socket=64   # 64 cores on each socket
...
RESULTS_DIR="/lustre/fsw/${SLURM_ACCOUNT}/${USERID}/results/wav2vec/LibriSpeech"

TRAIN_DATA="/lustre/fsw/${SLURM_ACCOUNT}/datasets/data/librispeech_sp_tarred"
...
VOCAB="CHAR"
STEPS="400k"
EXP_NAME="Wav2VecCTC_${FILTERS}_${NORM}_LIBRISPEECH_${VOCAB}_s${STEPS}"
...
CHECKPOINT_NAME="Wav2VecCTC_${FILTERS}_${NORM}_LIBRISPEECH_${VOCAB}.nemo"
...
&& CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python /code/speech_to_text_wav2vec.py \
	...
	model.train_ds.tarred_audio_filepaths="/data/train/audio__OP_0..511_CL_.tar" \
	...
	model.encoder.cfg.encoder.encoder_layers=12 \
	model.encoder.cfg.encoder.embedding_dim=${FILTERS} \
	model.encoder.cfg.encoder.encoder_layerdrop=.05 \
	model.encoder.cfg.encoder.ffn_embedding_dim=3072 \
	model.encoder.cfg.encoder.num_attention_heads=8 \	
